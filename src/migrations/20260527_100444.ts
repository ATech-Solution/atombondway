...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`DROP TABLE IF EXISTS \`plugins\`;`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "users_id", "media_id", "projects_id", "project_categories_id", "products_id", "product_categories_id") SELECT "id", "order", "parent_id", "path", "users_id", "media_id", "projects_id", "project_categories_id", "products_id", "product_categories_id" FROM \`payload_locked_documents_rels\`;`)
