...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`ALTER TABLE \`hero_content\` DROP COLUMN \`cta_secondary_href\`;`)
  await db.run(sql`ALTER TABLE \`hero_content_locales\` DROP COLUMN \`cta_secondary_text\`;`)
}
